...
import AdminView from '@/views/admin_dashboard/AdminView.vue' // Import the admin component
...
const fetchUser = async () => {
  ...
  if (error) {
    ...
    isAdmin.value = data.user?.user_metadata?.role === 'admin'
  }
}
...
<template>
  <!-- Conditionally render AdminView or User Appointment based on user role -->
                ...
                <v-list v-if="schedules.length > 0">
                  <v-list-item v-for="schedule in schedules" :key="schedule.schedule_id">
                    ...
                    <v-list-item-subtitle
                      >with {{ schedule.medicalstaff.name }}</v-list-item-subtitle
                    >
                ...
                <v-list>
                  <v-list-item v-for="booked in bookedSchedules" :key="booked.appointment_id">
                    <v-list-item-title
                      ...
                      {{ booked.appointment_time }}</v-list-item-title
                    >
                    <v-list-item-subtitle
